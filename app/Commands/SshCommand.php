...
class SshCommand extends Command
{
    ...
    private function registerConnection($message = null)
    {
        if ($message) {
            $this->line($message);
        }
        ...
        foreach ($this->connections as $cn) {
            if ($cn['slug'] === $conn['slug']) {
                $this->registerConnection('Connection with same name has been found');
                return;
            }
        }
    ...
    private function cloneConnection($conn, $message = null)
    {
        if ($message) {
            $this->line($message);
        }
        $this->info('Register New Connection');
        $conn['name'] = $this->ask('name', $conn['name']);
        $conn['slug'] = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $conn['name'])));
        foreach ($this->connections as $cn) {
            if ($cn['slug'] === $conn['slug']) {
                $this->cloneConnection($conn, 'Connection with same name has been found');
                return;
            }
        }
        $conn['host'] = $this->ask('host', $conn['host']);
        $conn['port'] = $this->ask('port', $conn['port']);
        $conn['user'] = $this->ask('user', $conn['user']);
        $this->info('Authentication Type: public_key / password');
        $conn['type'] = $this->ask('type', $conn['type']);
        if (strtolower($conn['type']) === 'public_key') {
            $keys = scandir(getenv('HOME') . '/.ssh/');
            $matchedKeys = [];
            foreach ($keys as $key) {
        ...
        $this->info('Password: user password / key phasprase');
        $pass = $this->ask('password', null);
        if (is_null($pass) && !is_null($conn['password'])) {
            $conn['password'] = $pass;
        }
        $conn['port_forwarding'] = strtolower($this->ask('port_forwarding', $conn['port_forwarding']));
        if ($conn['port_forwarding'] === 'yes') {
            $this->info('Port Forwarding Method: local / remote');
            $conn['pf_method'] = strtolower($this->ask('pf_method', $conn['pf_method'] ?? 'local'));
            $conn['local_port'] = $this->ask('local_port', $conn['local_port'] ?? 8088);
            $conn['remote_port'] = $this->ask('remote_port', $conn['remote_port'] ?? 80);
        }
        $this->connections[] = $conn;
        file_put_contents($this->config, json_encode($this->connections));
